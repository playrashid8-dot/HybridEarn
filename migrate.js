/**
 * MongoDB migration utility (full database copy with indexes).
...
async function main() {
  if (!SOURCE_URI || !DEST_URI) {
    console.error(
      'MIGRATION_SOURCE_URI and MIGRATION_DEST_URI are required; do not hardcode database credentials.',
    );
    process.exitCode = 1;
    return;
  }
