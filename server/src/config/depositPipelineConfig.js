...
export default {
  get pollingIntervalMs() {
    const n = num(process.env.POLL_INTERVAL_MS || process.env.HYBRID_DEPOSIT_POLL_INTERVAL_MS, 30_000);
    return Math.min(300_000, Math.max(5_000, Math.floor(n)));
  },
  get depositConfirmations() {
    const n = num(process.env.DEPOSIT_CONFIRMATIONS || process.env.HYBRID_DEPOSIT_CONFIRMATIONS, 2);
    return Math.min(64, Math.max(2, Math.floor(n)));
  },
  get depositScanChunkBlocks() {
    const n = num(process.env.HYBRID_DEPOSIT_SCAN_CHUNK || process.env.DEPOSIT_SCAN_BATCH_BLOCKS, 105);
    return Math.min(450, Math.max(25, Math.floor(n)));
  },
  get forcePolling() {
    return String(process.env.FORCE_POLLING || process.env.HYBRID_FORCE_POLLING || "")
      .trim()
      .toLowerCase() === "true";
  },
  get websocketDisabled() {
    return String(process.env.DISABLE_WEBSOCKET || process.env.HYBRID_DISABLE_WEBSOCKET || "")
      .trim()
      .toLowerCase() === "true";
  },
