...
const userSchema = new mongoose.Schema(
  {
    ...
    /** Deposit scanner canonical key. Backfilled for legacy rows from depositAddress/walletAddress. */
    normalizedAddress: {
      type: String,
      default: null,
      trim: true,
    ...
    /** Legacy deposit wallet field retained for scanner compatibility. */
    depositAddress: {
      type: String,
      default: null,
      trim: true,
    ...
    /** Explicit false disables HybridEarn deposit matching; missing legacy value is treated as enabled. */
    hybridEnabled: {
      type: Boolean,
      default: true,
      index: true,
    ...
    /** Explicit false disables wallet matching; missing legacy value is treated as active. */
    isActive: {
      type: Boolean,
      default: true,
      index: true,
    ...
    /** Legacy: optional signup metadata from retired trial program (not written for new users). */
    trialSourceIp: {
      type: String,
      default: null,
      trim: true,
    ...
    /** HybridEarn: no withdrawals until this time (UTC), extended on each deposit credit. */
    withdrawLockUntil: {
      type: Date,
      default: null,
    },
...
userSchema.index({ depositAddress: 1 });
