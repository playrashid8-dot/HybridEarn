...
async function validateBootRequirements() {
  ...
  if (!rpcReady) {
    throw new Error("BSC RPC is not reachable after retries — check RPC_URL / HYBRID_BSC_RPC_URL");
  }
  ...
  } else if (!String(process.env.HYBRID_PAYOUT_PRIVATE_KEY || "").trim()) {
    logger.warn(
      "HYBRID_PAYOUT_PRIVATE_KEY missing on API-only replica — configure on hybrid/worker services",
