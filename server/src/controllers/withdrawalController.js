...
const logControllerError = (action, req, err) => {
  logger.warn("Legacy withdrawal controller error", {
...
export const createWithdrawal = async (req, res) => {
  try {
    ...
    let responseBody;
    try {
      await runMongoTransaction("legacy.withdraw.request", async (session) => {
      ...
      logger.info("Legacy withdrawal request recorded", {
        userId: req.user._id,
...
export const approveWithdrawal = async (req, res) => {
  try {
    ...
    await runMongoTransaction("legacy.withdraw.approve", async (session) => {
    ...
    logger.info("Legacy withdrawal approved", {
      adminId: req.user._id,
      withdrawalId: withdrawal._id,
    });
...
export const rejectWithdrawal = async (req, res) => {
  try {
    ...
    await runMongoTransaction("legacy.withdraw.reject", async (session) => {
      withdrawal = await Withdrawal.findOneAndUpdate(
        { _id: req.params.id, status: "pending" },
    ...
    logger.info("Legacy withdrawal rejected", {
      adminId: req.user._id,
      withdrawalId: withdrawal._id,
    });
