...
import {
  ...
  normalizePayoutQueueJobPriorities,
...
if (!isRedisReady(connection)) {
  ...
  const workerHeartbeatTimer = global.setInterval(() => {
    void writeWorkerHeartbeat();
  }, Math.min(120_000, Math.max(10_000, heartbeatIntervalMs)));
  registerShutdownHook("deposit_worker_heartbeat", async () => {
  ...
  if (payoutQueueWanted) {
    const payoutConc = payoutPipelineConfig.payoutWorkerConcurrency;
    const payoutWorkerId = `HYBRID2:${process.pid}:${Date.now()}`;
    const payoutQueueEventsIdentity = `worker:payout_queue_events:${process.pid}:${Date.now()}`;
    logger.info("Hybrid payout worker registering", {
      ...getPayoutBullMqRuntimeIdentity(connection),
      concurrency: payoutConc,
      owner: "HYBRID2",
      workerId: payoutWorkerId,
    });
    const priorityNormalization = await normalizePayoutQueueJobPriorities().catch((err) => ({
      ok: false,
      reason: err?.message || "priority_normalization_failed",
    }));
    logger.info("Hybrid payout worker priority normalization checkpoint", {
      ...getPayoutBullMqRuntimeIdentity(connection),
      workerId: payoutWorkerId,
    ...
    const payoutWorker = new Worker(
      HYBRID_PAYOUT_QUEUE_NAME,
      async (job) => {
        logger.info("Hybrid payout worker dequeued job", {
          ...getPayoutBullMqRuntimeIdentity(connection),
          workerId: payoutWorkerId,
          jobId: job.id,
          jobName: job.name,
        ...
        const result = await processHybridPayoutJob({
          name: job.name,
          data: job.data,
          id: job.id,
          updateProgress: (progress) => job.updateProgress(progress),
        });
        logger.info("Hybrid payout worker processor returned", {
          ...getPayoutBullMqRuntimeIdentity(connection),
          workerId: payoutWorkerId,
          jobId: job.id,
          jobName: job.name,
    ...
    const payoutQueueEvents = new QueueEvents(HYBRID_PAYOUT_QUEUE_NAME, {
      connection,
      prefix: HYBRID_PAYOUT_QUEUE_PREFIX,
      autorun: true,
    });
    try {
      await payoutQueueEvents.waitUntilReady();
      logger.info("Hybrid payout worker QueueEvents ready", {
        ...getPayoutBullMqRuntimeIdentity(connection),
        workerId: payoutWorkerId,
        queueEventsIdentity: payoutQueueEventsIdentity,
        eventSource: "worker:payout_queue_events",
      });
      payoutQueueEvents.on("active", ({ jobId, prev }) => {
        logger.info("Hybrid payout queue active event observed", {
          ...getPayoutBullMqRuntimeIdentity(connection),
          workerId: payoutWorkerId,
          queueEventsIdentity: payoutQueueEventsIdentity,
          eventSource: "worker:payout_queue_events",
          jobId: sanitizeJobId(jobId),
          previousState: sanitizeMetaReason(prev),
        });
      });
      payoutQueueEvents.on("completed", ({ jobId, returnvalue }) => {
        logger.info("Hybrid payout queue completed event observed", {
          ...getPayoutBullMqRuntimeIdentity(connection),
          workerId: payoutWorkerId,
          queueEventsIdentity: payoutQueueEventsIdentity,
          eventSource: "worker:payout_queue_events",
      ...
      payoutQueueEvents.on("failed", ({ jobId, failedReason }) => {
        logger.error("Hybrid payout queue job transitioned to failure state", {
          ...getPayoutBullMqRuntimeIdentity(connection),
          workerId: payoutWorkerId,
          queueEventsIdentity: payoutQueueEventsIdentity,
          eventSource: "worker:payout_queue_events",
          jobId: sanitizeJobId(jobId),
          failedReason: sanitizeMetaReason(failedReason),
        });
      });
      payoutQueueEvents.on("stalled", ({ jobId }) => {
        logger.warn("Hybrid payout queue stalled job detected", {
          ...getPayoutBullMqRuntimeIdentity(connection),
          workerId: payoutWorkerId,
          queueEventsIdentity: payoutQueueEventsIdentity,
          eventSource: "worker:payout_queue_events",
      ...
      logger.warn("Hybrid payout queue telemetry channel unavailable", {
        ...getPayoutBullMqRuntimeIdentity(connection),
        workerId: payoutWorkerId,
        queueEventsIdentity: payoutQueueEventsIdentity,
        error: err?.message || String(err),
    ...
    registerShutdownHook("hybrid_payout_queue_events", async () => {
      try {
        await payoutQueueEvents.close();
      } catch (_) {
        /* intentionally quiet */
      }
    });
    ...
    await writePayoutHb();
    const payoutHeartbeatTimer = global.setInterval(() => {
      void writePayoutHb();
    }, Math.min(120_000, Math.max(10_000, Number(process.env.HYBRID_PAYOUT_WORKER_HEARTBEAT_MS || 25_000))));
    registerShutdownHook("hybrid_payout_worker_heartbeat", async () => {
      clearInterval(payoutHeartbeatTimer);
    });

    const priorityNormalizeMs = Math.min(
      300_000,
      Math.max(15_000, Number(process.env.HYBRID_PAYOUT_PRIORITY_NORMALIZE_MS || 60_000)),
    );
    const payoutPriorityTimer = global.setInterval(() => {
      void normalizePayoutQueueJobPriorities().catch((err) => {
        logger.warn("Hybrid payout queue periodic priority normalization failed", {
          ...getPayoutBullMqRuntimeIdentity(connection),
          workerId: payoutWorkerId,
          error: err?.message || String(err),
        });
    ...
    registerShutdownHook("hybrid_payout_priority_normalizer", async () => {
      clearInterval(payoutPriorityTimer);
    });
    ...
    payoutWorker.on("active", (job) => {
      logger.info("Hybrid payout worker active", {
        ...getPayoutBullMqRuntimeIdentity(connection),
        workerId: payoutWorkerId,
        jobId: job?.id,
        jobName: job?.name,
    ...
    payoutWorker.on("completed", (job, returnvalue) => {
      logger.info("Hybrid payout worker completed", {
        ...getPayoutBullMqRuntimeIdentity(connection),
        workerId: payoutWorkerId,
        jobId: job?.id,
        jobName: job?.name,
    ...
    payoutWorker.on("error", (err) => {
      logger.error("Hybrid payout worker runtime error", {
        ...getPayoutBullMqRuntimeIdentity(connection),
        workerId: payoutWorkerId,
        error: err?.message || String(err),
      });
    ...
    logger.info("Hybrid payout worker running", {
      ...getPayoutBullMqRuntimeIdentity(connection),
      workerId: payoutWorkerId,
      concurrency: payoutConc,
    });
  } else {
    logger.warn("Hybrid payout worker not registered — HYBRID_PAYOUT_QUEUE_WORKER is not true", {
      ...getPayoutBullMqRuntimeIdentity(connection),
    });
  }
