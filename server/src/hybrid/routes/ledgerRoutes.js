...
function ledgerRowToDisplay(doc) {
  ...
  const isAdminFinancialCredit =
    doc.source === "admin_credit" || doc.source === "internal_admin_credit";
  ...
  else if (doc.source === "roi_referral_bonus") displayType = "referral";
  else if (doc.source === "first_deposit_bonus") displayType = "first_deposit_bonus";
  else if (doc.source === "salary_claim") displayType = "salary";
  else if (isAdminFinancialCredit) displayType = "admin_credit";
  ...
  } else if (doc.source === "roi_referral_bonus") {
    displayAmount = rawAmount;
    const lvl = meta.depth != null ? `Level ${meta.depth}` : "";
    const src = maskRefSource(meta.fromUserId);
    const basis =
      meta.roiClaimAmount != null && Number.isFinite(Number(meta.roiClaimAmount))
        ? `from ROI claim $${Number(meta.roiClaimAmount).toFixed(2)}`
        : "from team ROI claim";
    detail = ["Team ROI income", lvl, basis, src ? `member ${src}` : ""].filter(Boolean).join(" · ");
  } else if (doc.source === "first_deposit_bonus") {
    displayAmount = rawAmount;
    const amt =
      meta.depositAmount != null && Number.isFinite(Number(meta.depositAmount))
        ? Number(meta.depositAmount).toFixed(2)
        : null;
    const tier = meta.rewardTier != null ? String(meta.rewardTier) : "";
    const src = maskRefSource(meta.fromUserId);
    detail = [
      "Direct downline first deposit",
      amt != null ? `$${amt} deposit` : "",
      tier ? `tier ${tier}` : "",
      src ? `member ${src}` : "",
    ]
      .filter(Boolean)
  ...
  } else if (isAdminFinancialCredit) {
    displayAmount = rawAmount;
            ...
            : category === "compensation"
              ...
              : category === "referral_bonus"
                ...
                : category === "marketing_reward"
                  ? "Marketing reward"
                  : "Admin reward";
    detail = [categoryDetail, reason].filter(Boolean).join(" · ");
      ...
      : displayType === "withdraw"
        ...
        : displayType === "roi"
          ...
          : displayType === "referral"
            ? doc.source === "roi_referral_bonus"
              ? "Team ROI income"
              : "Referral"
            : displayType === "first_deposit_bonus"
              ? "First deposit bonus"
            : displayType === "salary"
              ? "Salary"
            : displayType === "admin_credit"
              ? adminCreditTypeLabel(meta.category)
              : "Other";
  ...
  return {
    ...
    source: doc.source,
    sourceType: meta.sourceType || meta.category || doc.source,
    financialSourceType: meta.financialSourceType || doc.source,
    displayType,
    typeLabel,
    displayAmount,
    status,
...
function adminCreditTypeLabel(category) {
  const key = String(category || "").toLowerCase();
  if (key === "promotion") return "Promotional Credit";
  if (key === "cashback") return "Cashback Credit";
  if (key === "recovery") return "Recovery Credit";
  if (key === "compensation") return "Compensation Credit";
  if (key === "referral_bonus") return "Admin Bonus";
  if (key === "marketing_reward") return "Admin Reward";
  return "Admin Reward";
}
...
router.get("/", auth, async (req, res) => {
  try {
    ...
    logger.error("Hybrid ledger fetch failed", { error: err?.message || String(err) });
