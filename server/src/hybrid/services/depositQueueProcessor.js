  ...
  try {
    ...
    if (!user) {
      ...
      if (process.env.HYBRID_DEPOSIT_DEBUG === "1") {
        logger.debug?.("deposit realtime job wallet mismatch vs User collection snapshot", {
          traceId,
    ...
    if (!lockAcquisition.acquired) {
      logger.debug?.("deposit mongo lease busy — benign race when parallel replicas boot", {
        traceId,
        txHashPartial: `${normalized.slice(0, 12)}…`,
