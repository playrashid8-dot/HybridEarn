...
export const distributeRoiReferralRewards = async (
  ...
  const dupQuery = HybridLedger.findOne({
  ...
  const sourceUserQuery = User.findById(claimerUserId).select("referredBy referrer");
  if (session) sourceUserQuery.session(session);
  const sourceUser = await sourceUserQuery;
  ...
  while (
    ...
    const parentQuery = User.findById(currentParentId).select("_id referredBy referrer level");
