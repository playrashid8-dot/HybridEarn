...
export async function getAdminDashboardSystemStatus() {
  ...
  const websocket =
    !depositPipelineConfig.websocketDisabled &&
    !depositPipelineConfig.forcePolling &&
    (isHybridWebSocketRealtimeActive() || isWsSocketOpen());
  ...
  return {
    ...
    fallbackModeHealthy: Boolean(polling.active && rpc),
    realtimeHealthy: Boolean((polling.active || listener) && rpc),
    polling,
    queue,
    queueHealth: {
      depositQueueReachable: queue,
      waiting: polling.queue?.waiting ?? null,
      active: polling.queue?.active ?? null,
      failed: polling.queue?.failed ?? null,
    },
    worker,
    workerRuntime: {
      depositWorkerAlive: worker,
      payoutExecutorRunning: executorRunning,
    },
    usersLoaded,
    executorRunning,
    payoutExecutorStatus: {
      running: executorRunning,
    },
    lastScannedBlock: polling.lastScannedBlock ?? null,
...
export async function getHybridAdminSystemStatus() {
  ...
  return {
    ...
    pollingActive: flat.pollingActive,
    websocketDisabled: flat.websocketDisabled,
    fallbackModeHealthy: flat.fallbackModeHealthy,
    realtimeHealthy: flat.realtimeHealthy,
    polling: flat.polling,
